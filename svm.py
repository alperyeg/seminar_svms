...
def load_data():
    ...
    data_container=np.zeros((len(data),len(data[0])))
    for i,item in enumerate(data):
        data_container[i]=[1.]+[float(j) for j in item[:len(data[0])-1]]
...
def L(alpha, y, x):
    return - np.sum(alpha) + 1./2 * np.dot(alpha * y, np.dot(k(x), alpha * y))
...
cls = [np.dot(w, x[:, i]) for i in xrange(len(y))]
srt = np.argsort(cls)
